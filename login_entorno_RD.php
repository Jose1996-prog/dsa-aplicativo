...
<body>
    <form action="login_entorno_RD.php" method="post">
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $con->prepare('SELECT * FROM users_entorno WHERE usuario = ?');
    if ($stmt) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($contrasenha == $user['contrasenha']) { // Comparación directa porque la contraseña está almacenada como int
                ...
                $_SESSION['entorno'] = $user['entorno'];
                header('Location: read&delete_entorno.php'); // Redirige al archivo que deseas
                exit;
            } else {
