...
<body>
    <form action="login_climatico_RD.php" method="post">
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $con->prepare('SELECT * FROM users_climatico WHERE usuario = ?');
    if ($stmt) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($contrasenha == $user['contrasenha']) { // Comparación directa porque la contraseña está almacenada como int
                ...
                $_SESSION['climatico'] = $user['climatico'];
                header('Location: read&delete_climatico.php'); // Redirige al archivo que deseas
                exit;
            } else {
