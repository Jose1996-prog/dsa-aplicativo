...
<body>
    <form action="login_alimentos_ins_upd.php" method="post">
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $con->prepare('SELECT * FROM users_alimentos WHERE usuario = ?');
    if ($stmt) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($contrasenha == $user['contrasenha']) { // Comparación directa porque la contraseña está almacenada como int
                ...
                $_SESSION['alimentos'] = $user['alimentos'];
                header('Location: edit_alimentos_ins_tec.php'); // Redirige al archivo que deseas
                exit;
            } else {
