...
if (!isset($_SESSION['user_id']) || !isset($_SESSION['produccion'])) {
    header('Location: login_produccion_upd.php');
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
    ...
    $update_query = "UPDATE asistencia_produccion_limpia SET 
            fecha = ?,
            encargado_visita = ?,
            cargo = ?,
            dimension = ?,
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'edit') {
    ...
    $query = "SELECT fecha,
            ...
            cargo_funcionario_entidad,
            nombre_funcionario_departamento,
            cargo_funcionario_departamento FROM asistencia_produccion_limpia WHERE id=?";
    ...
    if ($stmt = $con->prepare($query)) {
        ...
        if ($stmt->fetch()) {
            echo '<form action="update_produccion.php" method="post">';
            echo '<table>';
            ...
            echo '<td><input type="text" id="cargo_del_entrevistado" name="cargo_del_entrevistado" value="' . htmlspecialchars($cargo_del_entrevistado) . '" maxlength="50"></td>';
            echo '</tr>';
            ...
            echo '<td><input type="text" id="objetivo_de_la_visita" name="objetivo_de_la_visita" value="' . htmlspecialchars($objetivo_de_la_visita) . '" maxlength="100"></td>';
            echo '</tr>';
            ...
            echo '<td><input type="text" id="objeticargo_funcionario_entidadvo_de_la_visita"
...
$query = "SELECT * FROM asistencia_produccion_limpia";
...
<a href="logout_produccion_upd.php" class="logout-button">Cerrar sesión</a>
<body>
