...
if (!isset($_SESSION['user_id']) || !isset($_SESSION['sustancias'])) {
    header('Location: login_sustancias_ins_upd.php');
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
    ...
    $update_query = "UPDATE ins_sustancias_quimicas
                        SET
                            fecha = ?,
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'edit') {
    ...
    if ($stmt = $con->prepare($query)) {
        ...
        if ($stmt->fetch()) {
            echo '<form action="edit_sustancias_ins_tec.php" method="post">';
            echo '<table>';
...
$query = "SELECT * FROM ins_sustancias_quimicas";
...
<a href="logout_sustancias_ins_upd.php" class="logout-button">Cerrar sesión</a>
</head>
<body>
