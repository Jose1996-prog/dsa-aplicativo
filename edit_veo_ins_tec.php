...
if (!isset($_SESSION['user_id']) || !isset($_SESSION['veo'])) {
    header('Location: login_veo_ins_upd.php');
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'edit') {
    ...
    if ($stmt = $con->prepare($query)) {
        ...
        if ($stmt->fetch()) {
            echo '<form action="edit_veo_ins_tec.php" method="post">';
            echo '<table>';
...
<a href="logout_veo_ins_upd.php" class="logout-button">Cerrar sesión</a>
</head>
<body>
