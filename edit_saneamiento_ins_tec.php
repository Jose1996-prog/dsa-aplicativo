...
if (!isset($_SESSION['user_id']) || !isset($_SESSION['saneamiento'])) {
    header('Location: login_saneamiento_ins_upd.php');
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
    ...
    $update_query = "UPDATE ins_saneamiento
                        SET
                            fecha = ?,
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action']) && $_POST['action'] == 'edit') {
    ...
    if ($stmt = $con->prepare($query)) {
        ...
        if ($stmt->fetch()) {
            echo '<form action="edit_saneamiento_ins_tec.php" method="post">';
            echo '<table>';
...
$query = "SELECT * FROM ins_saneamiento";
...
<a href="logout_saneamiento_ins_upd.php" class="logout-button">Cerrar sesión</a>
</head>
<body>
